...
var reset = function() { 
    ...
    d3.select("#console").text("Put some points within the triangle..."); 
    ...
    var handleClick = function() {
        ...
        if (util.triangleContainsPoint(p, kirkpatrick.outer_triangle)) {
            ...
            if (!selfCrosses) { 
                ...
                points.push(p);
                d3.select("#console").text("The total number of points = " + (points.length + 3)); 
            } else { 
                d3.select("#console").text("No self-crosses lines are allowed!"); 
            }
        }
        else {
            d3.select("#console").text("It is outside of the triangle!"); 
        }
    };
...
buttons.closePolygon.on("click", function() { 
    if (points.length < 3) { 
        d3.select("#console").text("Please select at least three points in order to proceed!");
    } else { 
        ...
        if (selfCrosses) { 
            ...
            d3.select("#console").text("Waiting for it to be triangulated!");
...
buttons.triangulate.on("click", function() { 
    ...
    d3.select("#console").text("Waiting to identify the independent set!");
    buttons.triangulate.attr("disabled", "disabled"); 
...
buttons.findIndependentSet.on("click", function() { 
    ...
    d3.select("#console").text("Waiting to remove the independent set!");
    buttons.findIndependentSet.attr("disabled", "disabled"); 
...
var locatePoint = function() {
    ...
    step_count = 0;
    query = undefined;  
    d3.select("#console").text("Waiting for query point for selection!");
...
buttons.step.on("click", function() { 
    ...
    if (step_count >= len) { 
        ...
        d3.select("#console").text("The point is located!!");
